...
const CountryDropDown = ({ setCountry, country, isBig }) => {
    ...
    function CountriesList({ items }) {
        return items.map((item, index) => (
            ...
            >
                <div style={{ width: '5rem', height: '0rem', marginRight: '8px' }} className="rounded-full flex items-center justify-center">
                ...
                <label style={{ fontSize: '3rem', color: 'rgb(255, 223, 189)', fontFamily: 'SpoqaHanSansNeoBold', marginLeft: '6.4rem'}} className={`${(selectedTab === item.id) ? "text-white" : "text-gray-subNavbar"} font-spoqaBold cursor-pointer`}>{item.text}</label>            
            </div>
        ));
    }
    return (
