    ...
    function CountriesList({ items }) {
        return items.map((item, index) => (
            ...
            >
                ...
                <label style={{ fontSize: '2.4rem', color: 'rgb(255, 223, 189)', fontFamily: 'SpoqaHanSansNeoBold', marginLeft: '6.4rem'}} className={`${(selectedTab === item.id) ? "text-white" : "text-gray-subNavbar"} font-spoqaBold cursor-pointer`}>{item.text}</label>            
            </div>
        ));
    }
    return (
