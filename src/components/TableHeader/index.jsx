...
const TableHeader = ({ smallCard = false, isMinigame = false, middleText = "무", isFreeboard }) => {
    ...
    if (isMinigame === true) return (
        ...
        <div style={{ paddingTop: smallCard ? '1.69rem' : '1.8rem', paddingBottom: '1.375rem', display: 'flex', justifyContent: 'right', alignItems: 'center', paddingRight: smallCard ? '1.8rem' : '2rem', letterSpacing: '-0.07rem', color: 'rgb(204, 194, 182)', fontFamily: 'SpoqaHanSansNeoMedium', marginBottom: smallCard ? '' : '0.3rem' }} className="flex w-full pb-8 font-spoqaMedium text-14px tracking-tight text-gray-r454545">
            <div style={{ fontSize: '2.625rem', marginLeft: isFreeboard ? '2rem' : smallCard ? '' : '' }} className="h-full flex items-center justify-end">승(홈)</div>
            <div style={{ fontSize: '2.625rem', marginLeft: isFreeboard ? '13.5rem' : smallCard ? '13.5rem' : middleText === "VS" ? '13.75rem' : '14.45rem' }} className="h-full flex items-center justify-end">{middleText}</div>
            <div style={{ fontSize: '2.625rem', marginLeft: isFreeboard ? '12.2rem' : smallCard ? '13.2rem' : '13.2rem' }} className="h-full flex items-center justify-end">패(원정)</div>
            <div style={{ fontSize: '2.625rem', marginLeft: isFreeboard ? '9.2rem' : smallCard ? '10rem' : '10rem' }} className="h-full flex items-center justify-end">결과</div>
        </div>
    )
}
